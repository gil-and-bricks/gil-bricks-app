...
/**
 * A GOLDEN CAN RECORD A BUG AND LOCK IT IN (M5).
 *
 ...
 * ever right — and one of them is currently wrong.
 *
 * `zoopla-auction-terrace-floorplan.json` records
 *   floorPlanImageUrls: ["f0fc15a57a54093f7eac2ada9e51c05501595fa3.jpg"]
 * A BARE FILENAME. Every photograph in the same file is a full
 * `https://lid.zoocdn.com/...` URL. It is not a bad capture: the saved page
 * really does carry the floor plan as `{"filename": "<hash>.jpg"}` with no
 * absolute URL anywhere, and `zoopla-newbuild-semi-floorplan.json` records
 * three more the same way. The extractor never turns that filename into an
 * address.
 *
 * WHY IT MATTERS. `handoff.ts` only writes `fp` when the value matches
 * /^https:\/\//, so a bare filename is not carried across as a broken image —
 * it is DROPPED, silently. The effect is that no Zoopla listing has ever handed
 * a floor plan to the analyser, and nothing anywhere said so. The panel is
 * worse off still: it sets `floorplan.available` from the same value without
 * that filter, so it offers a measure tool over an address that cannot load.
 *
 * This gate asserts what the goldens never did: that a portal image address is
 * an address. It does NOT change the extractor — fixing that is a product
 * change and this sprint ships none. The two known-bad fixtures are written
 * down below, dated, and the gate fails if a THIRD appears or if a listed one
 * is quietly fixed without being struck off. That is the same ratchet the
 * inline-copy baseline uses: a number that may only go down.
 */
describe('portal image addresses are addresses, not filenames', () => {
  /**
   * Known bad TODAY, 2026-09-14, awaiting a product fix. Strike a line out when
   * the extractor is repaired — the test fails if you do not.
   */
  const KNOWN_BROKEN: Record<string, readonly string[]> = {
    'zoopla-auction-terrace-floorplan.json': ['floorPlanImageUrls'],
    'zoopla-newbuild-semi-floorplan.json': ['floorPlanImageUrls'],
  };

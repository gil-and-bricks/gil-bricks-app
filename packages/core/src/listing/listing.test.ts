...
/**
 * A GOLDEN CAN RECORD A BUG AND LOCK IT IN (M5) — AND ONE DID (M6 FIXED IT).
 *
 * The golden files were generated BY the extractor, so they prove one thing
 * only: that the output has not drifted. They cannot tell you the output was
 * ever right — and two of them were wrong.
 *
 * `zoopla-auction-terrace-floorplan.json` recorded
 *   floorPlanImageUrls: ["f0fc15a57a54093f7eac2ada9e51c05501595fa3.jpg"]
 * A BARE FILENAME, while every photograph in the same file was a full
 * `https://lid.zoocdn.com/...` URL. Zoopla really does carry floor plans as
 * `{"filename": "<hash>.jpg"}` with no absolute URL anywhere on the page, and
 * the extractor absolutised the photographs and not the plans. `handoff.ts`
 * then dropped the value, because it only carries an `fp` that is an https
 * URL — so NO Zoopla listing had ever handed a floor plan to the analyser.
 *
 * M6 fixed it: one function now turns filenames into addresses for photographs
 * and plans alike, using the prefix READ FROM THE PAGE. The ledger below is
 * therefore empty, and this gate asserts the invariant outright.
 */
/**
 * THE LEGAL-PACK WARNING (M6).
 *
 * A Rightmove listing that says "for sale by the Modern Method of Auction"
 * handed over to the analyser with no auction flag, so the board never warned
 * about the legal pack — while the panel, reading the same sentence with the
 * same config patterns, had already put "Auction sale" on screen. Someone could
 * commit to a reservation fee without ever being told to read the pack.
 *
 * These assert the flag from the LISTING'S OWN WORDS, and assert just as firmly
 * that a quiet listing is not flagged — a false auction warning on an ordinary
 * deal would be its own kind of wrong.
 */
describe('an auction is flagged on both portals, and a quiet listing is not', () => {
  it('Rightmove: the wording "Modern Method of Auction" sets the flag', () => {
    const { doc, url } = loadDoc('rightmove', 'rightmove-reduced-terrace-leasehold.html');
    const r = extractListing('rightmove', doc, FALLBACK_CONFIG, url);
    if (!r.ok) throw new Error('failed');
    expect(r.listing.description.value ?? '').toMatch(/Modern Method of Auction/i);
    expect(r.listing.isAuction).toEqual({ value: true, status: 'found' });
  });

  it('Zoopla: the portal\'s own flag still wins', () => {
    const { doc, url } = loadDoc('zoopla', 'zoopla-auction-terrace-floorplan.html');
    const r = extractListing('zoopla', doc, FALLBACK_CONFIG, url);
    if (!r.ok) throw new Error('failed');
    expect(r.listing.isAuction).toEqual({ value: true, status: 'found' });
  });
  ...
  ] as const)('%s/%s says nothing about an auction and is not flagged', (portal, file) => {
    const { doc, url } = loadDoc(portal, file);
    const r = extractListing(portal, doc, FALLBACK_CONFIG, url);
    if (!r.ok) throw new Error('failed');
    expect(r.listing.isAuction.value).not.toBe(true);
  });

  it.each([
    ['zoopla', 'zoopla-newbuild-semi-floorplan.html'],
    ['zoopla', 'zoopla-newhome-6bed-hmo-candidate.html'],
  ] as const)('%s/%s is an ordinary sale and is not flagged', (portal, file) => {
    const { doc, url } = loadDoc(portal, file);
    const r = extractListing(portal, doc, FALLBACK_CONFIG, url);
    if (!r.ok) throw new Error('failed');
    expect(r.listing.isAuction.value).not.toBe(true);
...
describe('portal image addresses are addresses, not filenames', () => {
  /**
   * EMPTY, and it stays empty. Both entries were struck out on 2026-09-14 when
   * the extractor was fixed — this gate is what said they could be. Anything
   * added here must carry a date and a reason.
   */
  const KNOWN_BROKEN: Record<string, readonly string[]> = {};

  const IMAGE_FIELDS = ['floorPlanImageUrls', 'photoUrls'] as const;
  const usable = (v: string): boolean => /^https:\/\//.test(v);

  it.each(FIXTURES)('$file: every image the handoff can carry is a real URL', ({ file }) => {
    const goldenName = file.replace(/\.html$/, '.json');
    const goldenPath = join(CORPUS, 'golden', goldenName);
    if (!existsSync(goldenPath)) return;
    const golden = JSON.parse(readFileSync(goldenPath, 'utf8')) as Record<string, { value: unknown }>;
    const expectedBroken = KNOWN_BROKEN[goldenName] ?? [];

    for (const field of IMAGE_FIELDS) {
      const value = golden[field]?.value;
      if (!Array.isArray(value) || value.length === 0) continue;
      const bad = (value as string[]).filter((v) => !usable(v));
      const isKnown = expectedBroken.includes(field);
      if (isKnown) {
        // Still broken, as recorded. If this ever passes, the product was fixed
        // and the ledger above is stale — say so rather than going quietly green.
        expect(
          bad.length,
          `${goldenName} ${field} is on the known-broken list but is now clean. `
          + 'Delete its line from KNOWN_BROKEN.',
        ).toBeGreaterThan(0);
      } else {
        expect(
          bad,
          `${goldenName} ${field} carries ${bad.length} value(s) that are not URLs. `
          + 'The handoff puts these in an <img src>, so they resolve against our own origin and break.',
        ).toEqual([]);
      }
    }
  });

  it('the known-broken ledger names only fixtures that exist', () => {
    for (const name of Object.keys(KNOWN_BROKEN)) {
      expect(existsSync(join(CORPUS, 'golden', name)), `${name} is on the ledger but not on disk`).toBe(true);
    }
  });
});
...
describe('extractors: golden output for every fixture', () => {
  ...
  it('marks genuinely-absent fields missing, not guessed', () => {
    ...
    /**
     * AND STILL NOT GUESSED. Rightmove has no structured auction field, so a
     * listing whose wording says nothing about an auction stays
     * `unavailable-on-this-portal` — never `found(false)`. Silence on a
     * Rightmove page is not evidence that a sale is not an auction, and saying
     * so would be inventing a fact. This fixture is the quiet one; the
     * modern-method-of-auction terrace is asserted the other way below.
     */
    const quiet = loadDoc('rightmove', 'rightmove-reduced-detached-freehold.html');
    const q = extractListing('rightmove', quiet.doc, FALLBACK_CONFIG, quiet.url);
    if (!q.ok) throw new Error('rm quiet failed');
    expect(q.listing.isAuction.status).toBe('unavailable-on-this-portal');

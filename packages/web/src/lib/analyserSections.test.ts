...
describe('section overview strip (N2)', () => {
  ...
  it('the comparables summary reads as one plain line, singular and plural', () => {
    expect(SECTION_STRIP.compsSummary(14, '£245/m²')).toBe('14 comparable sales · typical £245/m²');
    expect(SECTION_STRIP.compsSummary(1, '£245/m²')).toBe('1 comparable sale · typical £245/m²');
    expect(SECTION_STRIP.compsSummary(9, null)).toBe('9 comparable sales');
  });
});

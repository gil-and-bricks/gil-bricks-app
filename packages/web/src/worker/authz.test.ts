...
describe('deleting the account really deletes everything', () => {
  const USER_TABLES = ['deals', 'deal_facts', 'deal_verdicts', 'deal_stage_history',
    'deal_changes', 'deal_deaths', 'deal_floorplans', 'refurb_cue_seen',
    'bridging_enquiries', 'bridging_factfinds', 'tool_saves', 'saved_deals', 'users'] as const;
  ...
  const seedEverything = async (): Promise<void> => {
    ...
    sqlite.prepare("INSERT INTO refurb_cue_seen (user_id, cue_key, seen_at) VALUES ('userA', 'fixture-1', ?)").run('2026-01-01T00:00:00Z');

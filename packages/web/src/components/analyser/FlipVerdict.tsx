...
export function FlipVerdict({ config, comps, valuation }: {
  ...
  const nextSnapshot = analysis
    ? { score: deal ? deal.score : null, criteriaJson: JSON.stringify({ thresholds: requireThresholds(config), assumptions: p }), boardFigure: `${fmtMoney(analysis.profitAfterTax.value)} profit` }
    : null;
  useEffect(() => {

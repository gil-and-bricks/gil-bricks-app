...
export function BtlVerdict({ config, comps, valuation }: {
  ...
  const judged = judgedBy('btl', requireThresholds(config) as unknown as Record<string, number>);
  ...
  if (rentOk && comps && Number(s.price) > 0) {
    try {
      ...
      if (features.dealScore) {
        deal = scoreDeal('btl', inputs, soldEvidence, { customKeys: judged.customKeys });
      }
    } catch (err) {
  ...
  const nextSnapshot = analysis
    ? { soldEvidence: soldEvidence ?? null, score: deal ? deal.score : null, headline: deal ? deal.headline : '', criteriaJson: JSON.stringify({ thresholds: judged.thresholds, assumptions: p }), lever: analysis.lever ?? null, boardFigure: BTL_COPY.boardFigure(fmtMoney(analysis.cashflowAfterTax.value)) }
    : null;
  useEffect(() => {
  ...
  return (
    <section class="glass card" aria-labelledby="verdict-h">
      ...
      {deal && <DealScoreChip deal={deal} strategy="btl" evidence={analyserEvidence(soldEvidence !== undefined, null, 'btl')} />}

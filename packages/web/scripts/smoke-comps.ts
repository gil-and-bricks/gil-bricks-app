...
console.log(
  `valuation CF37 1DL (last sale £120k 2019-03, 90sqm) → est £${Math.round(v.estimate).toLocaleString('en-GB')} ` +
    ...
    `| confidence ${v.confidence}: ${v.confidenceReason} | lines: ${v.lines.map((l: { label: string; estimate: number }) => `${l.label} £${Math.round(l.estimate).toLocaleString('en-GB')}`).join('; ')} | as of ${v.asOf}`,
);

...
function builtPages(dir = DIST, out = []) {
  for (const name of readdirSync(dir)) {
    ...
    if (statSync(full).isDirectory()) {
      if (['_astro', 'map', 'brand', 'area-data', 'fonts'].includes(name)) continue;
      builtPages(full, out);
...
const withDeal = (p) => (/\/analyser\/$/.test(p) ? `${p}?${DEAL}` : p);
...
for (const [vpName, viewport, isMobile] of [['desktop', { width: 1440, height: 900 }, false], ['phone', { width: 390, height: 844 }, true]]) {
  ...
  for (const path of pages) {
    ...
    const say = (kind, text) => {
      ...
      if (excusedLocally(text)) return;
      noise.push(`${kind}: ${text.slice(0, 150)}`);
    };
    page.on('pageerror', (e) => say('threw', String(e.message ?? e)));
    page.on('console', (m) => {
      const t = m.text();
      if (/Content Security Policy/.test(t)) return say('BLOCKED BY OUR OWN CSP', t);
...
const scope = LOCAL ? ' (API routes not exercised: no Worker on the local server)' : '';

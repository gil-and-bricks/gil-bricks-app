...
| `dealFacts` | on | Facts and re-scoring on a saved deal (P5): add what happened — a builder's quote, a survey finding, a down-valuation, auction fees, a service charge or ground rent — and the deal re-runs the SAME `@gil-bricks/core` maths the analyser runs, writing a verdict snapshot each time. Non-numeric facts (a covenant, a short lease) flag the deal and never invent a cost. | Off: no "what happened" control and no fact list on the board, `/api/deals/:id/facts` is 404, and a deal keeps the score it was saved with. Facts already recorded stay in the database untouched. |

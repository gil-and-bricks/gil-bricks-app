...
| `bridgingVideo` | on | The **video slot on `/bridging-finance`** (S1): a click-to-load YouTube slot between the explanation and the enquiry form, holding the operator's video with the broker. Nothing is fetched from YouTube until the visitor presses play, and while `BRIDGING_VIDEO.url` is empty the slot says a video is coming rather than pretending to have one. | The section is not rendered at all. The explanation runs straight into the enquiry form exactly as it did before the slot existed, and no third-party markup or script ships with the page. |
| `epcRegisterLookup` | on | The analyser's floor-area button asks the **real EPC register** through `GET /api/epc`, a Worker route holding the register's bearer token server-side. It answers for any certified property, including one that has not sold in decades, and the figure is labelled "From the EPC register". If the register is unreachable or rate-limits us, the sold-data match is used as a fallback and labelled as such. | The button uses only the sold-data match it used before E1 — an EPC-joined floor area attached to a past sale in our monthly data — and says the figure came from a past sale, not the register. `/api/epc` answers 404 and the register is never called. Nothing is deleted; the `epc_cache` table simply stops being read. |
...
| `desktopMore` | on | A1. A More menu in the DESKTOP header, reading the same `NAV.more` list as the phone's bottom sheet minus what the desktop already shows: the primary row, Deals, the account (which the signed-in control beside the socials owns since A2), and Privacy and Terms (the footer's). Without it `/comparables` and `/credit` have no link in the desktop header at all — the tab bar that holds them is `display:none` above 640px. Config: `desktopMoreLinks()` in `src/config/nav.ts`. | Off: the header is Analyse, Area Data, Tools, Bridging finance, then Deals, then the socials and the account control. The phone sheet is untouched; `/comparables` and `/credit` become reachable on a desktop only by URL or from a strategy landing page. |
...
| `staleDataNote` | on | The honest line beside the footer's as-of date when the monthly data refresh has plainly stopped: once the manifest's own `generatedAt` is older than `DATA_FRESHNESS.staleAfterDays` (45 days, `src/config/freshness.ts`), the footer adds "This is older than usual — treat the figures as a rough guide." The age is computed by `dataFreshness` in `@gil-bricks/core`; the footer only formats it. | The as-of month still shows exactly as it always did, and the page says nothing about whether it is current — the pre-C3 behaviour, in which a frozen date looked identical to a fresh one. Nothing else changes: no data is withheld and no figure moves. |
| `stampDutyCost` | on | **Stamp duty as its own named cost tile** on all four analysers (E9): the tax named by the country whose rules applied (Stamp Duty / Land Transaction Tax), its own figure, a line saying which rules were used and whether the additional-property surcharge is included, a pointer to the "Buying as" control that changes it, and the band-by-band working in the usual maths accordion. It carries the `sec-costs` anchor, so the Costs chip lands on it. | The buy-to-let analyser shows the tax inside its cash-in tile exactly as it did before (total plus bands), and flip, BRRRR and HMO name it only inside a collapsed maths accordion — the pre-E9 behaviour, in which the operator went looking for it and could not find it. The `sec-costs` anchor returns to the cash-in / operating-costs tile. No figure changes either way: the tax was always in cash-in. |
| `homeVideo` | on | The homepage's **click-to-load video slot** (H1), the same component the bridging and credit pages use: nothing reaches YouTube until somebody presses play, and while `HOME.video.url` is empty it says a walkthrough is coming rather than pretending to have one. | The section is not rendered at all and no YouTube markup ships with the page. Everything else on the homepage is untouched. |
| `homeWhyFree` | on | The homepage's **"Why this is free"** section (H1). While `HOME.whyFree.body` is empty it shows a plainly-marked placeholder saying the operator is writing it; the moment `body` has words, they replace the placeholder with no code change. | The section is not rendered. Nothing else moves — it is the last thing on the page before the trust line. |
| `subjectLookup` | on | The **Google lookup for the subject property** (E10), in the property card under the address the person typed: a new-tab search pre-filled with their postcode, house number and flat number. With a postcode but no house number it shows "Add the house number to look this property up." instead of a button — a bare postcode is a street, and searching one as though it were their property would be a quiet lie. It searches only what they typed; no street is inferred from the sales that share the postcode. | No lookup on any analyser. The comparable rows and the map popups still offer their own Google search exactly as before, and nothing else changes. |

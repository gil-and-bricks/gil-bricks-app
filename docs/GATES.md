...
| **Deal pack** | `npm run pack-gate -w packages/web` | Sign in → save a real BRRRR deal → press the pack button on its card → complete the declaration → set a business name, a logo and an accent colour → build the pack. Then, MEASURED IN THE BROWSER: six sheets at A4 (none overflowing), the accent as the COMPUTED colour of the rules and headings, every image a data URI, no request to a portal host, no Deal Score and no verdict in the text, the locked boxes disabled AND still in the document when the disabling is forced off, the builder's controls gone under print emulation, and the saved file reopened from disk with the network refused. | Whether the words are the right words. It checks that the compliance block is PRESENT and carries what was declared, never that a particular registration is valid. |

...
| 3 | Shared engines | The ONE ComparablesEngine, ONE ValuationEngine, config-driven DealAnalyser shell, maths lib + effective-dated rates.json. | Unit tests pass for every locked definition, incl. SDLT/LTT band cases (England +5% surcharge; Welsh standalone table). | In progress — S3.1 maths; S3.2 rates engine; S3.3 ComparablesEngine live on R2; ValuationEngine next |

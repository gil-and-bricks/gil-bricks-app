...
| 2 | Data pipeline | Public-repo GitHub Actions job builds monthly per-postcode-sector JSON + E&W .pmtiles into R2, stamped by manifest.json. | Fresh manifest.json in R2 with correct dataAsOf; spot-checked sector files carry schemaVersion; monthly run completes on free tier. | **Done** — S2.1 schema; S2.2 full E&W build; S2.3 EPC £/sqm live (93.7% matched); E&W pmtiles shipped in S7.1. Parquet was in the original objective, was never built, and is **abandoned** (ruled 2026-09-07) |
...
| 11 | Launch | Final name + domain, DNS cutover, go-live checks. | Production domain live over HTTPS; £0/month bill confirmed. | **Done (DM1)** — PropLaunch is live at https://proplaunch.ai; the data bucket is on data.proplaunch.ai; the old workers.dev address 301s every page and still serves /api for already-installed extensions |
...
- **S6.2** — 2026-08-31 — Saved deals (idempotent, 100 cap, ownership), Kit consent outbox (inline push + 15-min cron, backoff×5), My deals with WhatsApp share: **complete**.
- **S7.1** — 2026-08-31 — Self-hosted dark basemap (E&W PMTiles z14 on R2, own glyphs/sprites), lime radius + price-scaled synced pins + clustering, glass popups, transaction mini-map, deferred 257KB-gz map chunk: **complete**.

...
| 3 | Shared engines | The ONE ComparablesEngine, ONE ValuationEngine, config-driven DealAnalyser shell, maths lib + effective-dated rates.json. | Unit tests pass for every locked definition, incl. SDLT/LTT band cases (England +5% surcharge; Welsh standalone table). | In progress — S3.1 maths lib done (40 tests); SDLT/LTT + rates.json next |
